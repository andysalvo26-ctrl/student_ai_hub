...
The workflow began with a Google Sheet exported as CSV (`data/registry/SAIH Content - Corpus v0.csv`). Each row represents one human-approved source with the following fields:
...
### Snapshot Files (`data/snapshots/corpus_snapshots/`)
...
### Stable Snapshots (`data/snapshots/corpus_snapshots_stable/`)
...
### Corpus Index (`data/indexes/corpus_index_stable.json`)
...
### Chunk Files (`data/chunks/chunks.jsonl`)
